...
class ACLAction extends SugarBean
{
    ...
    public static function hasAccess($is_owner=false, $in_group=false, $access = 0, ACLAction $action = null)
    {
        ...
        */
        if ($access != 0 && (
            ...
            || ($in_group && $access == ACL_ALLOW_GROUP)  || ($in_group && $access == ACL_ALLOW_GROUP_AND_GROUP_CHILD)//need to pass if in group with access somehow
        )) {
            $GLOBALS['log']->fatal('oke 1');
            return true;
        }
    ...
    */
    ...
    {
        //$GLOBALS['log']->fatal('userHasAccess');
        global $current_user;
        ...
        if (ACLAction::getUserAccessLevel($user_id, $category, 'access', $type) < ACL_ALLOW_ENABLED) {
            //$GLOBALS['log']->fatal('getUserAccessLevel');
            return false;
        }
        if (empty($_SESSION['ACL'][$user_id][$category][$type][$action])) {
            //$GLOBALS['log']->fatal('empty action');
            ACLAction::getUserActions($user_id, false);
        }
        ...
        if (!empty($_SESSION['ACL'][$user_id][$category][$type][$action])) {
            ...
            */
            //$GLOBALS['log']->fatal($_SESSION['ACL'][$user_id][$category][$type][$action]['aclaccess']);
