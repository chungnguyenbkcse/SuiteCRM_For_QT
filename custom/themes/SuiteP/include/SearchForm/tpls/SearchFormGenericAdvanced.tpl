...
<script>
    {literal}
        var action = $("#search_form [name='action']").val();
        if (action != 'unaeffect'){
            $("[name='call_status_lead_advanced[]']").change(function() {
                var call_status_lead_id = $("[name='call_status_lead_advanced[]']").val();
                console.log(call_status_lead_id)
                console.log(action)
                $.ajax({
                    url: "index.php?module=Leads&entryPoint=get_call_status_description_leads",
                    data: {id: call_status_lead_id},
                    success: function(data){
                        console.log(data);
                        $("[name='call_status_description_lead_advanced[]']").html(data);
                    },
                    dataType: 'html'
                });
            })
            ...
            $("[name='call_status_description_lead_advanced[]']").remove();
            $("[for='call_status_description_lead_advanced']").remove();
        }
    $(document).ready(function () {
        ...
        if (role == "disbursement") {
            $("[name='call_status_description_lead_advanced[]']").remove();
            $("[for='call_status_description_lead_advanced']").remove();
            $("[name='call_status_lead_advanced[]']").remove();
            $("[for='call_status_lead_advanced']").remove();

            
        }
        else if (role == "sale") {
            $("[name='call_status_disbursement_advanced[]']").remove();
            $("[for='call_status_disbursement_advanced']").remove();
        }
