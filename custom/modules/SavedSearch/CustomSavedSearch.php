...
class SavedSearch extends SugarBean
{
    ...
    public function getForm($module, $inline = true, $orderBySelectOnly = false)
    {
        ...
        if (isset($_REQUEST['module'])){
            if ($_REQUEST['module'] == 'Leads'){
                if (isset($_REQUEST['saved_search_select'])) {    
                    $filter_for_leads_id = $_REQUEST['saved_search_select'];
                    $user = BeanFactory::getBean('Users', $current_user->id);

                    $query_saved_search_users = "SELECT COUNT(*) AS total FROM saved_search_users WHERE saved_search_id = '{$filter_for_leads_id}'";
                    ...
                    if ($total['total'] > 0) {
                        if ($user->is_admin == 0){
                            $sugarSmarty->assign('CHECK_DELETE', 'false');
                        }
                    }
                }
            }
        ...
        return $sugarSmarty->fetch('custom/modules/SavedSearch/CustomSavedSearchForm.tpl');
    }
    ...
    public function getSelect($module, &$savedSearchData = null)
    {
        ...
        if ($module == 'Leads') {
            ...
            WHERE
              deleted = \'0\' AND
              (assigned_user_id = \'' . $current_user->id . '\' OR assigned_user_id = \'1\') AND
              search_module =  \'' . $module . '\'
            ORDER BY name';
            $result = $db->query($query, true, "Error filling in saved search list: ");
    ...
    public function returnSavedSearch($id, $searchFormTab = 'advanced_search', $showDiv = 'no')
    {
        ...
        if(isset($_COOKIE["action"])) {
            echo "Cookie named '" . $_COOKIE["action"] . "' is not set!";
            $header="Location: index.php?action={$_COOKIE['action']}&module=";
        }
    ...
    public function handleDelete($id)
    {
        ...
        $query = "UPDATE saved_search_users SET deleted = '1' WHERE saved_search_id = '{$id}'";
        ...
        if(isset($_COOKIE["action"])) {
            $return_action = $_COOKIE["action"];
            SugarApplication::headerRedirect("Location: index.php?action={$return_action}&module={$requestSearchModule}&advanced={$_REQUEST['advanced']}&query=true&clear_query=true");
        }
        else {
            SugarApplication::headerRedirect("Location: index.php?action=disbursement&module={$requestSearchModule}&advanced={$_REQUEST['advanced']}&query=true&clear_query=true");
        }
    }
